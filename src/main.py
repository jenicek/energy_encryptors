...
sns.histplot(df.iloc[:, 1:].values.flatten(), bins=50, kde=True, ax=ax)
...
sample_house = df.sample(1).iloc[:, 1:].T
...
fig, ax = plt.subplots(figsize=(12, 6))
mean_consumption = df.iloc[:, 1:].mean()
...
sample_df = df.sample(n=100, random_state=42).iloc[:, 1:6]
...
fig, ax = plt.subplots(figsize=(8, 6))
sns.histplot(df.iloc[:, 1:].mean(axis=1), bins=50, kde=True, ax=ax)
...
pca_result = pca.fit_transform(df.iloc[:, 1:].dropna())
fig, ax = plt.subplots(figsize=(8, 6))
